    ...
    <div class="add-berita-card">
        <div class="add-berita-header">
            <div>
                ...
                <p>Ubah detail berita untuk ditampilkan pada halaman publik.</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.berita.update', $berita->id) }}" enctype="multipart/form-data" class="add-berita-form">
            @csrf
            @method('PUT')
            @if($errors->any())
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input
                        type="date"
                        name="published_at"
                        value="{{ old('published_at', $berita->published_at ? \Carbon\Carbon::parse($berita->published_at)->format('Y-m-d') : '') }}"
                    />
                </div>
                ...
                <div class="form-group full">
                    <label>Cover Image Saat Ini</label>
                    @if($berita->cover_image_url)
                        <div style="margin-bottom: 12px;">
                            <img src="{{ $berita->cover_image_url }}" alt="Cover" style="max-width: 300px; border-radius: 8px; border: 1px solid #e5e7eb;">
                        </div>
                    @endif
                    <label>Ganti Cover Image (opsional)</label>
